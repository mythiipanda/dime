...
from backend.config import Errors, CURRENT_SEASON # Removed DEFAULT_TIMEOUT as not directly used
...
async def get_team_stats_endpoint( # Renamed for clarity
    ...
    season: Optional[str] = Query(None, description=f"NBA season in YYYY-YY format (e.g., '2023-24') for dashboard stats. Defaults to {CURRENT_SEASON} in logic.", regex=r"^\d{4}-\d{2}$"),
    ...
    # Pass all query params; logic function will handle defaults if None
...
@router.get(
    "/{team_identifier}/info_roster", # Combined endpoint name for clarity
...
async def get_team_info_and_roster_endpoint( # Renamed for clarity
    team_identifier: str = Path(..., description="Team name, abbreviation, or ID."),
    season: Optional[str] = Query(None, description=f"NBA season (YYYY-YY). Defaults to {CURRENT_SEASON} in logic.", regex=r"^\d{4}-\d{2}$"),
...
async def get_team_passing_stats_endpoint( # Renamed for clarity
    team_identifier: str = Path(..., description="Team name, abbreviation, or ID."),
    season: Optional[str] = Query(None, description=f"Season (YYYY-YY). Defaults to {CURRENT_SEASON} in logic.", regex=r"^\d{4}-\d{2}$"),
    season_type: Optional[str] = Query(None, description="Season type. Logic default: 'Regular Season'."),
...
async def get_team_rebounding_stats_endpoint( # Renamed for clarity
    team_identifier: str = Path(..., description="Team name, abbreviation, or ID."),
    season: Optional[str] = Query(None, description=f"Season (YYYY-YY). Defaults to {CURRENT_SEASON} in logic.", regex=r"^\d{4}-\d{2}$"),
    season_type: Optional[str] = Query(None, description="Season type. Logic default: 'Regular Season'."),
...
async def get_team_shooting_stats_endpoint( # Renamed for clarity
    team_identifier: str = Path(..., description="Team name, abbreviation, or ID."),
    season: Optional[str] = Query(None, description=f"Season (YYYY-YY). Defaults to {CURRENT_SEASON} in logic.", regex=r"^\d{4}-\d{2}$"),
    season_type: Optional[str] = Query(None, description="Season type. Logic default: 'Regular Season'."),
    per_mode: Optional[str] = Query(None, description="Per mode. Logic default: 'PerGame'."),
