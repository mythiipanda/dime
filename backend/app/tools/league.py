...
async def text_to_sql(question: str) -> dict[str, Any]:
    ...
    examples = (
        ...
        "Q: Which teams have the most back-to-backs this season?\n"
        "Note silver_schedule date values are ISO timestamps; compare "
        "calendar days with date_diff after casting substr(date, 1, 10).\n"
        "SQL: WITH d AS (SELECT DISTINCT team_abbreviation, "
        "substr(date, 1, 10) AS gdate FROM silver_schedule "
        "WHERE _season = '2025-26'), o AS (SELECT team_abbreviation, gdate, "
        "LAG(gdate) OVER (PARTITION BY team_abbreviation ORDER BY gdate) "
        "AS prev FROM d) SELECT team_abbreviation, COUNT(*) AS b2b FROM o "
        "WHERE date_diff('day', CAST(prev AS DATE), CAST(gdate AS DATE)) = 1 "
        "GROUP BY 1 ORDER BY b2b DESC LIMIT 5"
    )
    feedback = ""
    for _ in range(3):
        try:
