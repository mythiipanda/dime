...
# --- Main Logic Function ---
@lru_cache(maxsize=TEAM_REBOUNDING_TRACKING_CACHE_SIZE)
def fetch_team_rebounding_stats_logic(
    ...
    if season_type not in _TEAM_REB_VALID_SEASON_TYPES:
        return format_response(error=Errors.INVALID_SEASON_TYPE.format(value=season_type, options=", ".join(list(_TEAM_REB_VALID_SEASON_TYPES)[:5])))
    if per_mode not in _TEAM_REB_VALID_PER_MODES:
        return format_response(error=Errors.INVALID_PER_MODE.format(value=per_mode, options=", ".join(list(_TEAM_REB_VALID_PER_MODES)[:5])))
