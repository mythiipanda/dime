...
# --- Main Logic Function ---
@lru_cache(maxsize=GAME_FINDER_CACHE_SIZE)
def fetch_league_games_logic(
    ...
    Args:
        player_or_team_abbreviation (str): 'P' for player or 'T' for team. Defaults to 'T'.
        player_id_nullable (Optional[int]): Player ID.
        team_id_nullable (Optional[int]): Team ID.
        season_nullable (Optional[str]): Season in 'YYYY-YY' format.
        season_type_nullable (Optional[str]): Season type (e.g., 'Regular Season').
        league_id_nullable (Optional[str]): League ID. Defaults to NBA.
    ...
    """
    logger.info(f"Executing fetch_league_games_logic with P/T: {player_or_team_abbreviation}, TeamID: {team_id_nullable}, PlayerID: {player_id_nullable}, Season: {season_nullable}, League: {league_id_nullable}, Dates: {date_from_nullable}-{date_to_nullable}")
    ...
    try:
        ...
        try:
            games_df = _apply_date_filters_to_df(games_df, date_from_nullable, date_to_nullable)
             ...
             return format_response(error=Errors.PROCESSING_ERROR.format(error=str(filter_error)))
