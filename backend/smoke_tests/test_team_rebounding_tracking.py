...
async def main():
    logger.info("--- Testing Team Rebounding Stats ---")
    ...
    for case in test_cases:
        ...
        try:
            ...
            if "error" in data:
                ...
                logger.info(f"SUCCESS for {case['description']}: Fetched team rebounding stats.")
                if "overall" in data:
                    logger.info(f"  Overall keys: {list(data['overall'].keys()) if data['overall'] else 'None'}")
                if "by_shot_type" in data:
                    logger.info(f"  By Shot Type count: {len(data['by_shot_type'])}")
                if "by_contest" in data:
                    logger.info(f"  By Contest count: {len(data['by_contest'])}")
                if "by_shot_distance" in data:
                    logger.info(f"  By Shot Distance count: {len(data['by_shot_distance'])}")
                if "by_rebound_distance" in data:
                    logger.info(f"  By Rebound Distance count: {len(data['by_rebound_distance'])}")
