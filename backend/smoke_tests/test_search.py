...
def test_search_players_dataframe():
    ...
    if os.path.exists(PLAYER_SEARCH_CSV_DIR):
        csv_files = [f for f in os.listdir(PLAYER_SEARCH_CSV_DIR) if f.startswith(f"player_search_{SAMPLE_PLAYER_QUERY.lower()}")]
...
def test_search_teams_dataframe():
    ...
    # Check if the CSV files were created
    if os.path.exists(TEAM_SEARCH_CSV_DIR):
        csv_files = [f for f in os.listdir(TEAM_SEARCH_CSV_DIR) if f.startswith(f"team_search_{SAMPLE_TEAM_QUERY.lower()}")]
        print(f"\nCSV files created: {csv_files}")
...
def test_search_games_dataframe():
    ...
    # Check if the CSV files were created
    if os.path.exists(GAME_SEARCH_CSV_DIR):
        clean_query = SAMPLE_GAME_QUERY.replace(" ", "_").replace(".", "").lower()
        clean_season_type = SAMPLE_SEASON_TYPE.replace(" ", "_").lower()
        csv_files = [f for f in os.listdir(GAME_SEARCH_CSV_DIR) if f.startswith(f"game_search_{clean_query}_{SAMPLE_SEASON}_{clean_season_type}")]
        print(f"\nCSV files created: {csv_files}")
