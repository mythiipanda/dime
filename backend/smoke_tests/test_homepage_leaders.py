"""
Smoke tests for the homepage_leaders module using real API calls.

These tests verify that the homepage leaders API functions work correctly
...
from api_tools.homepage_leaders import (
    fetch_homepage_leaders_logic,
    get_homepage_leaders,
    _get_csv_path_for_homepage_leaders,
...
class TestHomePageLeadersReal(unittest.TestCase):
    ...
    def test_get_csv_path_for_homepage_leaders(self):
        """Test that the CSV path is generated correctly."""
        # Test with default parameters
        path = _get_csv_path_for_homepage_leaders()
        self.assertIn("homepage_leaders_league00", path)
        ...
        self.assertIn("statPoints", path)
        self.assertIn("gameSeason", path)
        self.assertIn("HomePageLeaders", path)
        ...
        path = _get_csv_path_for_homepage_leaders(
            ...
            stat_category=StatCategory.rebounds,
            game_scope_detailed=GameScopeDetailed.last_10,
            data_set_name="LeagueAverage"
        )
        self.assertIn("homepage_leaders_league10", path)
        ...
        self.assertIn("statRebounds", path)
        self.assertIn("gameLast_10", path)
        self.assertIn("LeagueAverage", path)

    def test_fetch_homepage_leaders_logic_json(self):
        """Test fetching homepage leaders in JSON format."""
        # Call the function with real API
        json_response = fetch_homepage_leaders_logic(
            league_id="00",
            season="2024-25"
        )
        ...
        if "error" in data:
            ...
            if "HomePageLeaders" in data_sets:
                leaders_data = data_sets["HomePageLeaders"]
                self.assertIsInstance(leaders_data, list)
                self.assertGreater(len(leaders_data), 0)  # Should have team leaders
                print(f"Homepage Leaders count: {len(leaders_data)}")
                print(f"Sample leader: {leaders_data[0] if leaders_data else 'No data'}")

    def test_fetch_homepage_leaders_logic_dataframe(self):
        """Test fetching homepage leaders in DataFrame format."""
        # Call the function with real API
        json_response, dataframes = fetch_homepage_leaders_logic(
            league_id="00",
            season="2024-25",
        ...
        if "error" in data:
            ...
            for data_set_name, df in dataframes.items():
                ...
                # Verify expected columns for homepage leaders
                if data_set_name == "HomePageLeaders":
                    expected_columns = ["RANK", "TEAM_ID", "TEAM_NAME", "TEAM_ABBREVIATION", "PTS"]
            ...
            for data_set_name in dataframes.keys():
                csv_path = _get_csv_path_for_homepage_leaders(
                    ...
                    PlayerScope.all_players, StatCategory.points, GameScopeDetailed.season,
                    data_set_name
                )
    ...
    def test_fetch_homepage_leaders_logic_csv_cache(self):
        ...
        csv_paths = []
        for data_set_name in ["HomePageLeaders", "LeagueAverage", "LeagueHigh"]:
            csv_path = _get_csv_path_for_homepage_leaders(
                params["league_id"], params["season"], SeasonTypePlayoffs.regular,
                PlayerOrTeam.team, PlayerScope.all_players, StatCategory.points,
        ...
        _, dataframes1 = fetch_homepage_leaders_logic(**params, return_dataframe=True)
        ...
        _, dataframes2 = fetch_homepage_leaders_logic(**params, return_dataframe=True)
    ...
    def test_get_homepage_leaders(self):
        """Test the get_homepage_leaders function."""
        # Call the function with real API
        json_response, dataframes = get_homepage_leaders(
            league_id="00",
            season="2024-25",
            return_dataframe=True
        )
    ...
    def test_100_percent_parameter_coverage(self):
        """Test 100% parameter coverage for HomePageLeaders endpoint."""
        print("\n=== Testing 100% Parameter Coverage ===")

        # Test parameter combinations that work
        test_cases = [
            # Basic combinations
            {"league_id": "00", "season": "2024-25", "stat_category": StatCategory.points},
            {"league_id": "00", "season": "2023-24", "stat_category": StatCategory.points},
            {"league_id": "00", "season": "2022-23", "stat_category": StatCategory.points},

            # Different stat categories
            {"league_id": "00", "season": "2024-25", "stat_category": StatCategory.rebounds},
            {"league_id": "00", "season": "2024-25", "stat_category": StatCategory.assists},
                        ...
                        if dataframes:
                            for data_set_name, df in dataframes.items():
                                ...
                                csv_path = _get_csv_path_for_homepage_leaders(
                                    ...
                                    test_case.get("stat_category", StatCategory.points),
