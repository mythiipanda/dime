"""
Smoke tests for the leaders_tiles module using real API calls.

These tests verify that the leaders tiles API functions work correctly
...
from api_tools.leaders_tiles import (
    fetch_leaders_tiles_logic,
    get_leaders_tiles,
    _get_csv_path_for_leaders_tiles,
...
class TestLeadersTilesReal(unittest.TestCase):
    ...
    def test_get_csv_path_for_leaders_tiles(self):
        """Test that the CSV path is generated correctly."""
        # Test with default parameters
        path = _get_csv_path_for_leaders_tiles()
        self.assertIn("leaders_tiles_league00", path)
        ...
        self.assertIn("statPTS", path)
        self.assertIn("gameSeason", path)
        self.assertIn("LeadersTiles", path)
        ...
        path = _get_csv_path_for_leaders_tiles(
            ...
            stat="REB",
            data_set_name="CurrentSeasonLeaders"
        )
        self.assertIn("leaders_tiles_league00", path)
        ...
        self.assertIn("statREB", path)
        self.assertIn("gameLast_10", path)
        self.assertIn("CurrentSeasonLeaders", path)

    def test_fetch_leaders_tiles_logic_json(self):
        """Test fetching leaders tiles in JSON format."""
        # Call the function with real API
        json_response = fetch_leaders_tiles_logic(
            league_id="00",
            season="2024-25",
            stat="PTS"
        )
        ...
        if "error" in data:
            ...
            self.assertEqual(data["parameters"]["stat"], "PTS")
            ...
            if "CurrentSeasonLeaders" in data_sets:
                current_leaders = data_sets["CurrentSeasonLeaders"]
                self.assertIsInstance(current_leaders, list)
                self.assertEqual(len(current_leaders), 5)  # Should have top 5 teams
                print(f"Current Season Leaders count: {len(current_leaders)}")
                print(f"Sample leader: {current_leaders[0] if current_leaders else 'No data'}")

    def test_fetch_leaders_tiles_logic_dataframe(self):
        """Test fetching leaders tiles in DataFrame format."""
        # Call the function with real API
        json_response, dataframes = fetch_leaders_tiles_logic(
            league_id="00",
            season="2024-25",
            stat="REB",
        ...
        if "error" in data:
            ...
            self.assertEqual(data["parameters"]["league_id"], "00")
            self.assertEqual(data["parameters"]["season"], "2024-25")
            self.assertEqual(data["parameters"]["stat"], "REB")
            ...
            for data_set_name, df in dataframes.items():
                ...
                # Verify expected columns for leaders tiles
            ...
            for data_set_name in dataframes.keys():
                csv_path = _get_csv_path_for_leaders_tiles(
                    GameScopeDetailed.season, "00", PlayerOrTeam.team,
                    PlayerScope.all_players, "2024-25", SeasonTypePlayoffs.regular,
                    "REB", data_set_name
                )
    ...
    def test_fetch_leaders_tiles_logic_csv_cache(self):
        ...
        for data_set_name in data_set_names:
            csv_path = _get_csv_path_for_leaders_tiles(
                GameScopeDetailed.season, params["league_id"], PlayerOrTeam.team,
                PlayerScope.all_players, params["season"], SeasonTypePlayoffs.regular,
                params["stat"], data_set_name
            )
        ...
        _, dataframes1 = fetch_leaders_tiles_logic(**params, return_dataframe=True)
        ...
        _, dataframes2 = fetch_leaders_tiles_logic(**params, return_dataframe=True)
    ...
    def test_get_leaders_tiles(self):
        """Test the get_leaders_tiles function."""
        # Call the function with real API
        json_response, dataframes = get_leaders_tiles(
            league_id="00",
            season="2024-25",
            stat="PTS",
            return_dataframe=True
    ...
    def test_100_percent_parameter_coverage(self):
        """Test 100% parameter coverage for LeadersTiles endpoint."""
        print("\n=== Testing 100% Parameter Coverage ===")
        
        # Test parameter combinations that work
        test_cases = [
            # Basic combinations
            {"league_id": "00", "season": "2024-25", "stat": "PTS"},
            {"league_id": "00", "season": "2023-24", "stat": "PTS"},
            {"league_id": "00", "season": "2022-23", "stat": "PTS"},
            
            # Different stats
            {"league_id": "00", "season": "2024-25", "stat": "REB"},
            {"league_id": "00", "season": "2024-25", "stat": "AST"},
            {"league_id": "00", "season": "2024-25", "stat": "STL"},
            {"league_id": "00", "season": "2024-25", "stat": "BLK"},
            {"league_id": "00", "season": "2024-25", "stat": "FG_PCT"},
            
            # Player scope
            {"league_id": "00", "season": "2024-25", "stat": "PTS", "player_or_team": PlayerOrTeam.player},
            
            # Different game scopes
            {"league_id": "00", "season": "2024-25", "stat": "PTS", "game_scope_detailed": GameScopeDetailed.last_10},
            
            # Playoffs
            {"league_id": "00", "season": "2023-24", "stat": "PTS", "season_type_playoffs": SeasonTypePlayoffs.playoffs},
        ]
            ...
            try:
                ...
                if "error" in data:
                    ...
                    if "parameters" in data:
                        ...
                        self.assertEqual(params["stat"], test_case["stat"])
