...
from backend.api_tools.player_aggregate_stats import (
...
def test_fetch_player_stats_dataframe():
    ...
    if os.path.exists(PLAYER_STATS_CSV_DIR):
        csv_files = [f for f in os.listdir(PLAYER_STATS_CSV_DIR) if f.startswith(SAMPLE_PLAYER_NAME.lower().replace(" ", "_"))]
