"""
Smoke tests for the player_game_logs module.
Tests fetching player game logs.

This module tests the PlayerGameLogs endpoint with various parameters
...
from backend.api_tools.player_game_logs import (
    fetch_player_game_logs_logic,
    _get_csv_path_for_player_game_logs,
    VALID_SEASON_TYPES,
    VALID_PER_MODES,
    VALID_MEASURE_TYPES
)
...
# --- Test Functions ---
def test_fetch_player_game_logs_basic():
    """Test fetching player game logs with default parameters."""
    print("\n=== Testing fetch_player_game_logs_logic (basic) ===")

    # Test with default parameters (JSON output)
    json_response = fetch_player_game_logs_logic(
        SAMPLE_SEASON,
        DEFAULT_SEASON_TYPE,
        DEFAULT_PER_MODE,
...
def test_fetch_player_game_logs_player_id():
    """Test fetching player game logs with player ID filter."""
    print("\n=== Testing fetch_player_game_logs_logic with player ID filter ===")

    # Test with player ID filter
    json_response = fetch_player_game_logs_logic(
        SAMPLE_SEASON,
        DEFAULT_SEASON_TYPE,
        DEFAULT_PER_MODE,
    ...
    if "error" in data:
        ...
        assert params.get("player_id_nullable") == PLAYER_ID, f"player_id should be {PLAYER_ID}"
        print(f"Successfully fetched data for player ID: {PLAYER_ID}")
        ...
        for data_set_name, data_set in data_sets.items():
            if data_set and len(data_set) > 0:
                ...
                # Verify all records are for the specified player
                if "PLAYER_ID" in data_set[0]:
                    player_ids = set(game.get("PLAYER_ID") for game in data_set)
                    print(f"Player IDs in data: {player_ids}")
                    assert str(PLAYER_ID) in [str(pid) for pid in player_ids], "All records should be for the specified player"

    print("\n=== Player ID filter test completed ===")
    return data

def test_fetch_player_game_logs_team_id():
    """Test fetching player game logs with team ID filter."""
    print("\n=== Testing fetch_player_game_logs_logic with team ID filter ===")
...
def test_fetch_player_game_logs_date_range():
    """Test fetching player game logs with date range filter."""
    print("\n=== Testing fetch_player_game_logs_logic with date range filter ===")
...
def test_fetch_player_game_logs_different_season_types():
    """Test fetching player game logs with different season types."""
    print("\n=== Testing fetch_player_game_logs_logic with different season types ===")

    # Test each season type
    season_types = list(VALID_SEASON_TYPES.keys())
    results = {}

    for season_type in season_types:  # Test only valid season types
...
def test_fetch_player_game_logs_dataframe():
    """Test fetching player game logs with DataFrame output."""
    print("\n=== Testing fetch_player_game_logs_logic with DataFrame output ===")

    # Test with return_dataframe=True
    result = fetch_player_game_logs_logic(
        SAMPLE_SEASON,
        DEFAULT_SEASON_TYPE,
        DEFAULT_PER_MODE,
        DEFAULT_MEASURE_TYPE,
...
def test_fetch_player_game_logs_invalid_params():
    """Test fetching player game logs with invalid parameters."""
    print("\n=== Testing fetch_player_game_logs_logic with invalid parameters ===")
...
def run_all_tests():
    ...
    print(f"=== Running player_game_logs smoke tests at {datetime.now().isoformat()} ===\n")
    ...
    try:
        # Run basic tests
        run_test(test_fetch_player_game_logs_basic, "Basic Player Game Logs")

        # Run filter tests
        run_test(test_fetch_player_game_logs_player_id, "Player ID Filter")
        run_test(test_fetch_player_game_logs_team_id, "Team ID Filter")
        run_test(test_fetch_player_game_logs_date_range, "Date Range Filter")

        # Run parameter tests
        run_test(test_fetch_player_game_logs_different_season_types, "Different Season Types")

        # Run DataFrame and error tests
        run_test(test_fetch_player_game_logs_dataframe, "DataFrame Output")
        run_test(test_fetch_player_game_logs_invalid_params, "Invalid Parameters")
